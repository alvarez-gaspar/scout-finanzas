      ...
      <form onSubmit={submit} className="bg-white rounded-xl border shadow-sm p-6 space-y-4">
        ...
        <div>
          ...
          <input type="date" value={form.fecha_nacimiento}
            ...
            className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-violet-600" />
          {edad && <p className="text-xs text-violet-700 font-medium mt-1">{edad}</p>}
        </div>
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Etapa</label>
            <input type="text" value={form.etapa} placeholder="ej: Pionero I"
              onChange={e => setForm(f => ({ ...f, etapa: e.target.value }))}
              className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-violet-600" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Comunidad</label>
            <input type="text" value={form.comunidad} placeholder="ej: Los Cóndores"
